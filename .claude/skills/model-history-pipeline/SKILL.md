...
1. **Download full-history PBFs** → `osm_versions.parquet` + `osm_changes.parquet`
   ...
   **Advance `download.osm.end_date` before a refit** to the current cutoff (e.g. the Overture release date) so the fit sees the latest edits — it is the `osmium time-filter` upper bound, and a stale value silently caps the observation window (`start_date` stays `2016-01-01`). This is a standing monthly step; see [skills/full-data-pull](../full-data-pull/SKILL.md) step 1.
