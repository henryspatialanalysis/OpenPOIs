...
- **August 2026 restructures** (release `2026-08-19.0`, caught by `compare_taxonomy.py --strict` before the 2026-09 pull): `lifestyle_services/beauty_service` renamed to `personal_or_beauty_service` (allowlist + crosswalk already carried both; the stale crosswalk row was dropped); `food_delivery_service` moved from `shopping/food_and_beverage_store` to `lifestyle_services/food_service` (new `(L0,L2)` crosswalk row keeps it at Fast Food); `place_of_worship` moved up a level under `cultural_and_historic` (absorbed by the existing cascade, 452k POIs). The release also grew +9.3% month-over-month, ~864k of which the conflation-time internal dedup removed again.
